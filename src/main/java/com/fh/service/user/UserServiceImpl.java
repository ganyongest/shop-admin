   ...
   //登陆成功之后 把经常使用  但不经常改变的数据放在缓存中  方便用的时候直接取
    private void writeCache(HttpServletRequest request, User us,HttpServletResponse response) {
      ...
      //  request.getSession().setAttribute(SystemConstant.CURRENT_RESOURCELIST,resourceList);
        String resourListString = JSON.toJSONString(resourceList);
        RedisUtil.set(SystemConstant.CURRENT_RESOURCELIST+sessionId,resourListString);
        ...
        String allResourceString = JSON.toJSONString(allResourceList);
        RedisUtil.set(SystemConstant.ALL_RESOURCELIST+sessionId,allResourceString);
    ...
    public void loginOut(HttpServletRequest request, HttpServletResponse response) {
       ...
       // request.getSession().invalidate();
        String sessionId = SessionUtil.getSessionId(request, response);
        String[] keys = {SystemConstant.LOGGIN_CURRENT_USER+sessionId,SystemConstant.ALL_RESOURCELIST+sessionId,SystemConstant.CURRENT_RESOURCELIST+sessionId};
    ...
    public ServerResponse sendCode(String phone,HttpServletRequest request,HttpServletResponse response) {
        ...
        if(StringUtils.isNotBlank(phone)){
            ...
            if(StringUtils.isNotBlank(code)){
                ...
                String sessionId = SessionUtil.getSessionId(request, response);
                RedisUtil.setEx(SystemConstant.CODE+sessionId,code,SystemConstant.CODE_OUT_TIME);
                //验证码发送成功
                flag =1;
            }else{
                //验证码发送失败
    ...
    @Override
    public ServerResponse updatePassword(User user, String code, HttpServletRequest request,HttpServletResponse response) {
         //首先验证验证码是否正确
       // String sessionCode = (String) request.getSession().getAttribute(SystemConstant.CODE);
        String sessionId = SessionUtil.getSessionId(request, response);
        String sessionCode = RedisUtil.get(SystemConstant.CODE + sessionId);
