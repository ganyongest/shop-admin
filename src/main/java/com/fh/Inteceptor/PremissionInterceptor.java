...
public class PremissionInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
       ...
       // List<Resource> resourceList = (List<Resource>) request.getSession().getAttribute(SystemConstant.CURRENT_RESOURCELIST);
        String resourceListString = RedisUtil.get(SystemConstant.CURRENT_RESOURCELIST + sessionId);
        List<Resource> resourceList = JSONObject.parseArray(resourceListString, Resource.class);
        //获取所有的资源
       // List<Resource> allResourceList = (List<Resource>) request.getSession().getAttribute(SystemConstant.ALL_RESOURCELIST);
        String allResourceListString = RedisUtil.get(SystemConstant.ALL_RESOURCELIST + sessionId);
        List<Resource> allResourceList = JSONObject.parseArray(allResourceListString, Resource.class);
